...
class APGDAttack():
    ...
    def __init__(
        ...
        self.eps_orig = None if eps is None else eps + 0.
    ...
    def check_oscillation(self, x, j, k, y5, k3=0.75):
        ...
        for counter5 in range(k):
          t = t + (x[j - counter5] > x[j - counter5 - 1]).float()
    ...
    def normalize(self, x):
        if self.norm == 'Linf':
            ...
            out=x / (t.view(-1, *([1] * self.ndims)) + 1e-12)
            return out
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if self.norm == 'Linf':
            ...
            x_adv = x + self.eps.detach().clone() * torch.ones_like(x
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                ...
                grad = grad + torch.autograd.grad(loss, [x_adv])[0].detach()
            else:
        ...
        step_size = alpha * self.eps.detach().clone() * torch.ones([x.shape[0], *(
              ...
              if counter3 == k:
                  if self.norm in ['Linf', 'L2']:
                      ...
                      if fl_oscillation.sum() > 0:
                          ...
                          step_size[ind_fl_osc] = step_size[ind_fl_osc] / 2.0
                      ...
                      step_size[~fl_redtopk] = step_size[~fl_redtopk] / adasp_redstep
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        """
        if len(self.eps.shape)<4:
            self.eps=(self.eps[:,None,None,None]).expand(x.shape)
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                ...
                x_to_fool=x
                y_to_fool=y
                # self.logger.info('position in 0 allocate memory: {}'.format(torch.cuda.memory_allocated()))
                ...
                # self.logger.info('position in 1 allocate memory: {}'.format(torch.cuda.memory_allocated()))
                _, acc_curr, _, adv_curr = res_curr
                acc_curr=acc_curr.detach().clone()
                adv_curr=adv_curr.detach().clone()
                del res_curr
    ...
    def decr_eps_pgd(self, x, y, epss, iters, use_rs=True):
        ...
        if not use_rs:
            ...
            x_init = x_init + L1_projection(x, x_init - x, 1. * float(epss[0]))
        ...
        for eps, niter in zip(epss, iters):
            ...
            #
            if not x_init is None:
                x_init = x_init + L1_projection(x, x_init - x, 1. * eps)
...
class APGDAttack_targeted(APGDAttack):
    ...
    def dlr_loss_targeted(self, x, y):
        ...
        upper=-(x[u, y] - x[u, self.y_target])
        lower=(x_sorted[:, -1] - .5 * (x_sorted[:, -3] + x_sorted[:, -4]) + 1e-12)
        return upper / lower
