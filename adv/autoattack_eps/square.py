...
class SquareAttack():
        ...
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                ...
                for i_iter in range(self.n_queries):
                    ...
                    rd_ch=self.random_choice([c, 1, 1]).unsqueeze(0).repeat(x.shape[0],1,1,1)

                    new_deltas = torch.zeros([x.shape[0], c, h, w]).to(self.device)
                    new_deltas[idx_to_fool, :, vh:vh + s, vw:vw + s] = 2. * self.eps[idx_to_fool, :, vh:vh + s, vw:vw + s] * rd_ch[idx_to_fool]
                    ...
                    #     1. - idx_improved) * loss_min_curr)[idx_to_fool]
                    ...
                    ind_succ=margin_min <= 0.
                    torch.distributed.barrier()
                    ind_succ_all_list = [torch.zeros_like(ind_succ) for _ in range(torch.distributed.get_world_size())]
                    torch.distributed.all_gather(ind_succ_all_list, ind_succ)
                    torch.distributed.barrier()
                    ind_succ_all=torch.cat(ind_succ_all_list)
                    if ind_succ_all.nonzero().squeeze().numel()==n_ex_total*torch.distributed.get_world_size():
                        break
    ...
    def perturb(self, x, y=None):
        ...
        for counter in range(self.n_restarts):
            ...
            # ind_to_fool=torch.arange(x.shape[0])
            ...
            _, adv_curr = self.attack_single_run(x, y)
