        ...
        while counter_restarts < 1:
            ...
            while counter_iter < self.n_iter:
                with torch.no_grad():
                    ...
                    adv[ind_adv] = (x1 * (t < res2).float().reshape([-1, *[1]*self.ndims]) + adv * (t >= res2).float().reshape([-1, *[1]*self.ndims]))[ind_adv]
                    res2[ind_adv] = (t * (t < res2).float() + res2 * (t >= res2).float())[ind_adv]
                    x1[ind_adv] = (im2 + (x1 - im2) * self.beta)[ind_adv]
        ...
        with torch.no_grad():
            ...
            if not self.targeted:
                for counter in range(self.n_restarts):
                    ...
                    eps_new=self.eps.permute(1,2,3,0)[0,0,0,:].squeeze()
                    acc_curr = torch.max(acc_curr, res > eps_new)
                ...
                for target_class in range(2, self.n_target_classes + 2):
                    ...
                    for counter in range(self.n_restarts):
                        ...
                        # if len(ind_to_fool.shape) == 0: ind_to_fool = ind_to_fool.unsqueeze(0)
                        # if ind_to_fool.numel() != 0:
                        x_to_fool, y_to_fool = x.detach().clone(), y.detach().clone()
