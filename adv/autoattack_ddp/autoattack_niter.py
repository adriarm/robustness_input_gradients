...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, n_restarts=5, n_iter=200, verbose=False,
            ...
            self.fab = FABAttack_PT(self.model, n_restarts=5, n_iter=200, eps=self.epsilon, seed=self.seed,
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=200, verbose=False,
            ...
            self.fab = FABAttack_TF(self.model, n_restarts=5, n_iter=200, eps=self.epsilon, seed=self.seed,
                norm=self.norm, verbose=False, device=self.device)
        ...
        with torch.no_grad():
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    y_adv[false_batch] = output[false_batch].detach().to(x_adv.device)
