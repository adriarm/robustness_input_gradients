...
def main(args):
    ...
    print('Top5 acc of clean images is: {0:>7.4f}'.format(clean_acc['top5']))
    for eps_int in [1,2,4]:
        ...
        print('Top1 alt acc of eps {0} is: {1:>7.4f}'.format(eps_int, robust_acc['top1_alt']))
        print('Top5 alt acc of eps {0} is: {1:>7.4f}'.format(eps_int, robust_acc['top5_alt']))
...
def adv_validate(model, loader, args, eps_int, log_suffix='robust acc'):
    ...
    top1_m = AverageMeter()
    loss_m = AverageMeter()
    top1_alt_m = AverageMeter()
    top5_alt_m = AverageMeter()
    eps=eps_int/255
    model.eval()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        for attack_type in args.attack_types:
            ...
            with torch.no_grad():
                ...
                acc1_alt, acc5_alt = accuracy(output, target, topk=(1, 5))
        ...
        if args.distributed:
            ...
            acc1_alt = reduce_tensor(acc1_alt, args.world_size)
            acc5_alt = reduce_tensor(acc5_alt, args.world_size)
        torch.cuda.synchronize()

        top1_m.update(acc1.item(), output.size(0))
        loss_m.update(loss.item(), output.size(0))
        top1_alt_m.update(acc1_alt.item(), output.size(0))
        top5_alt_m.update(acc5_alt.item(), output.size(0))
        batch_time_m.update(time.time() - end)
        end = time.time()
        ...
        print(
            ...
            'AccAlt@1: {top1_alt.val:>7.4f} ({top1_alt.avg:>7.4f})  '
            'AccAlt@5: {top5_alt.val:>7.4f} ({top5_alt.avg:>7.4f})  '.format(
                log_name, batch_idx, last_idx, batch_time=batch_time_m, top1=top1_m, loss=loss_m, top1_alt=top1_alt_m, top5_alt=top5_alt_m))

    metrics = OrderedDict([('top1', top1_m.avg), ('advloss', loss_m.avg), ('top1_alt', top1_alt_m.avg), ('top5_alt', top5_alt_m.avg)])
