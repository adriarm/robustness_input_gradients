...
def main(args, args_text):
    ...
    if args.down > 1:
        down = torchvision.transforms.Resize(224//args.down, torchvision.transforms.InterpolationMode.NEAREST, antialias=False)
        up = torchvision.transforms.Resize(224, torchvision.transforms.InterpolationMode.NEAREST, antialias=False)
        loader_train.dataset.transform = torchvision.transforms.Compose([*loader_train.dataset.transform.transforms, down, up])
        loader_eval.dataset.transform = torchvision.transforms.Compose([*loader_eval.dataset.transform.transforms, down, up])
    print(loader_train.dataset.transform)
    print(loader_eval.dataset.transform)
    ...
    if args.advtrain:
        ...
        reg_loss_fn = input_norm_losses.DBPPow(p=args.p)
