...
def main(args):
    ...
    # model = prn_cifar.PreActResNet18().cuda()
    if args.distributed:
    ...
    dataset_eval.transform = torchvision.transforms.ToTensor()
    sampler_eval=None
    if args.distributed:
...
def adv_validate(model, loader, args, eps_int, log_suffix='robust acc'):
    ...
    for attack_type in args.attack_types:
        ...
        elif attack_type == 'pgd50':
            from adv.adv_utils import PgdAttackEps #pgd_attack
            attackers[attack_type]= PgdAttackEps(model, args.batch_size, None, None, eps, 1/255, 50, num_restart=5) #pgd_attack
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        for attack_type in args.attack_types:
            ...
            elif attack_type == 'pgd100' or attack_type == 'pgd50' or attack_type == 'pgd10':
